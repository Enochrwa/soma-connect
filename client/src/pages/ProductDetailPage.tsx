...
export default function ProductDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 grid md:grid-cols-2 gap-8">
      <div>
        <img src={p.images?.[0]} alt={p.title} className="w-full rounded-2xl aspect-square object-cover" />
      ...
      <div>
        ...
        <div className="text-sm text-slate/60 mt-1">★ {(p.avgRating ?? 0).toFixed(1)} · {p.reviewCount ?? 0} reviews</div>
        <div className="mt-4 flex items-baseline gap-3">
          <span className="price text-3xl text-saffron">{formatRWF(p.price)}</span>
          {p.comparePrice && <span className="text-slate/40 line-through font-mono">{formatRWF(p.comparePrice)}</span>}
        </div>
        <p className="mt-4 text-slate/80 leading-relaxed">{p.description}</p>
        <div className="mt-2 text-sm">
          {p.stock > 0 ? <span className="text-forest">✅ In stock ({p.stock})</span> : <span className="text-vermillion">❌ Out of stock</span>}
        </div>
        <div className="mt-6 flex flex-wrap gap-3">
          <button
            ...
            onClick={() =>
              dispatch(addItem({
                productId: p._id, title: p.title, image: p.images?.[0],
                unitPrice: p.price, quantity: 1, sellerId: String(p.sellerId?._id ?? p.sellerId),
                stock: p.stock,
              }))
            }
          >
